...
class InternalCalculator {
    ...
    // Get all of the materials that are required to build one unit of x, and if the material is not present in the current database then also add it to the list, it will go down to eventually materials that have no composite.
            ...
            switch (action) {
                ...
                case "2.1":
                    MaterialComposite toremove = new MaterialComposite(
                            new ArrayList<Material>(Server.currentlystored.materials()
                                    .stream().filter(x -> x.isExpired()).map(x -> x.clone()).collect(Collectors.toList())));
                ...
                case "2.2":
                    while (true) {
                        ...
                        TPS.addTransaction(new Transaction(TransactionType.ADD, inStockFromTemplate, "purchase_" + inStockFromTemplate.MaterialID()));
                ...
                case "2.3":
                    while (true) {
                        ...
                        TPS.addTransaction(new Transaction(TransactionType.REMOVE, inStockFromTemplate, "sell_" + inStockFromTemplate.MaterialID()));
                ...
                case "2.4":
                    while (true) {
                        ...
                        TPS.addTransaction(new Transaction(TransactionType.REMOVE, inStockFromTemplate, "steal_" + inStockFromTemplate.MaterialID()));
                ...
                case "2.5":
                    while (true) {
                        ...
                        CalculatorFunctions.getMaterialsForBuild(inStockFromTemplate, TPS.crunchData())
                            ...
                            TPS.addTransaction(new Transaction(TransactionType.REMOVE, x, "build_submaterial_" + x.MaterialID()));
                                });
                        built.addMaterial(inStockFromTemplate);
                        TPS.addTransaction(new Transaction(TransactionType.ADD, inStockFromTemplate, "built_" + inStockFromTemplate.MaterialID()));
