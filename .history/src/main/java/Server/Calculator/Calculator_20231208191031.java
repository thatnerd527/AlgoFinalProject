                    ...
                    built.materials().stream().forEach(x -> {
                        x.getComposite().materials().forEach(x2 -> {
                            ...
                            if (materialinstock.isPresent()) {
                                Material cloned = materialinstock.get().clone();
                                if (cloned.quantity >= x2.quantity) {
                                    cloned.quantity = x2.quantity;
                                    value -= cloned.quantity * cloned.valuePerQty;
                                    matdclone.removeMaterial(cloned);
                                } else {
