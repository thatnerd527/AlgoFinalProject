                ...
                case "1.5":
                    ...
                    built.materials().stream().forEach(x -> {
                        x.getComposite().materials().forEach(x2 -> {
                            Optional<Material> materialinstock = Server.currentlystored.materials().stream().filter(x3 -> x3.MaterialID().intValue() == x2.MaterialID().intValue()).findFirst();
                            if (materialinstock.isPresent()) {
                                if (materialinstock.get().quantity >= x2.quantity) {
