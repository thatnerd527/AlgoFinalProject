                ...
                case "4":
                    while (true) {
                        ...
                        if (isComposite) {
                            ...
                            while (true) {
                                boolean add = new Menu()
                                .withTitle("Material Composite adder\n| Amount of materials in composite: " + composite.getMaterials().size())
                                .withChoice("+", "Add material to composite")
                                .withChoice("F", "Finish")
                                        .makeASelection(wW, wR).equals("+");
                                if (!add) {
                                    builder.withComposite(composite);
                                    break;
                                }
                                while (true) {
                                    HashMap<String, String> result = new InputForm(wR, wW)
                                    .withField("Material ID")
                                    .withField("Amount (above 0)")
                                    .withTitle("Add material to composite")
                                    .receiveInput();
                                    try {
                                        if (Double.valueOf(result.get("Amount (above 0)")) <= 0) {
                                            throw new Exception();
                                        }
                                    } catch (Exception e) {
                                    ...
                                    Optional<Material> mat = Server.templatematerials,xxxxxxxxxxxxldlxzsx
                                }
