...
public class InventoryFunctions {
                            ...
                            while (true) {
                                ...
                                Optional<Material> getmat = Server.Server.templatematerials.materials()
                                        ...
                                        .filter(x -> result2.get("Template ID")
                                                .equals(Integer.valueOf(x.MaterialID())
                                                        .toString()))
                                        .findFirst();
                                if (!getmat.isPresent()) {
    ...
    public static Material GetTemplateMaterial(WrappedReader wR, WrappedWriter wW) {
        while (true) {
            ...
            Optional<Material> getmat = Server.Server.templatematerials.materials()
                    .stream()
                    .filter(x -> result.get("Template ID").equals(Integer.valueOf(x.MaterialID())
                            .toString()))
                    .findFirst();
            if (!getmat.isPresent()) {
                wW.write("Invalid template ID.\n");
    ...
    public static Material GetInStockMaterial(WrappedReader wR, WrappedWriter wW) {
        while (true) {
            ...
            Optional<Material> getmat = Server.Server.currentlystored.materials()
                    .stream()
                    .filter(x -> result.get("Material ID").equals(Integer.valueOf(x.MaterialID())
                            .toString()))
                    .findFirst();
            if (!getmat.isPresent()) {
    ...
    public static Material GetInStockMaterial(WrappedReader wR, WrappedWriter wW, MaterialComposite current) {
        while (true) {
            HashMap<String, String> result = new InputForm(wR, wW)
                    .withField("Material ID", true)
                    .withTitle("Get in stock material by ID")
                    .receiveInput();
            Optional<Material> getmat = current.materials()
                    .stream()
                    .filter(x -> result.get("Material ID").equals(Integer.valueOf(x.MaterialID())
                            .toString()))
                    .findFirst();
    ...
    public static Material ModifyMaterialQuantity(WrappedReader wR, WrappedWriter wW, Material m) {
        while (true) {
            HashMap<String, String> result = new InputForm(wR, wW)
            ...
            try {
                ...
                m.quantity = Double.valueOf(result.get("Quantity"));
                return m;
            } catch (Exception e) {
    ...
    public static Material ModifyMaterialDifferentiator(WrappedReader wR, WrappedWriter wW, Material m) {
        while (true) {
            String choice = new Menu()
                    .withTitle("Do you want to change the differentiator?")
                    .withChoice("Y", "Yes")
                    .withChoice("N", "No")
                    .makeASelection(wW, wR);
            if (choice.equals("N")) {
            ...
            HashMap<String, String> result = new InputForm(wR, wW)
                    .withField("Differentiator", true)
                    .withTitle("Differentiator")
                    .receiveInput();
            clone.differentiator = result.get("Differentiator");
            return clone;
        }
    }

    public static Material ModifyMaterialCost(WrappedReader wR, WrappedWriter wW, Material m) {
        while (true) {
            String choice = new Menu()
                    .withTitle("Do you want to change the overall cost / cost per quantity?")
                    .withChoice("T", "Total cost")
                    .withChoice("Q", "Cost per quantity")
                    .withChoice("N", "No")
                    .makeASelection(wW, wR);
            if (choice.equals("N")) {
                return m;
                ...
                case "T":
                    while (true) {
                        HashMap<String, String> result = new InputForm(wR, wW)
                                ...
                                .receiveInput();
                        try {
                            if (Double.valueOf(result.get("Cost")) <= 0) {
                                throw new Exception();
                            }
                            m.overrideValue = Double.valueOf(result.get("Cost"));
                            return m;
                        } catch (Exception e) {
                            wW.write("Invalid Cost value.\n");
                            continue;
                        }
                    }
                case "Q":
                    while (true) {
                        HashMap<String, String> result = new InputForm(wR, wW)
                                .withField("Cost per quantity", true)
                                .withTitle("Cost per quantity")
                                .receiveInput();
                        try {
                            if (Double.valueOf(result.get("Cost per quantity")) <= 0) {
                                throw new Exception();
                            }
                            m.setValuePerQty(Double.valueOf(result.get("Cost per quantity")));
                            return m;
                        } catch (Exception e) {
                            wW.write("Invalid Cost per quantity value.\n");
                            continue;
                        }
                    }
